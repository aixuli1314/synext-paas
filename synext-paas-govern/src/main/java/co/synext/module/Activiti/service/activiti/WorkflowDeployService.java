...
//      String[] processKeys = {"salary", "applyFlow", "expense", "CountSalary"};
    ...
    /**
     * 第二种：部署classpath/processes下的流程定义
     * @param exportDir
     * @param processKey
     * @throws Exception 
     */
    public void deployFromClassPathBpmn(String exportDir, String... processKey) throws Exception{
    	ResourceLoader resourceLoader = new DefaultResourceLoader();
//      String[] processKeys = {"salary.bpmn", "applyFlow.bpmn", "expense.bpmn", "CountSalary.bpmn"};
...
//	            Deployment deployment = repositoryService.createDeployment().addZipInputStream(zis).deploy(); //此方法会重复部署，不管资源变没变化
    ...
    private void deploySingleProcessByBpmn(ResourceLoader resourceLoader, String processKey, String exportDir) throws IOException {
    	...
    	String classpathResourceUrl = "classpath:/processes/" + processKey;
    	//Resource resource = resourceLoader.getResource(classpathResourceUrl);
    	//if(resource.exists()){
    		//InputStream inputStream = resource.getInputStream();
	        //if (inputStream == null) {
	        ...
	        //	String resourceName = resource.getFilename();
	        	DeploymentBuilder builder = repositoryService.createDeployment();
	        	builder.addClasspathResource("processes/"+processKey); //使用addClasspathResource方法将流程描述文件和流程图部署到数据库中
	        //	builder.addInputStream(resourceName, inputStream);
	        	String key = processKey.substring(0,processKey.lastIndexOf("."));
	        	String flowName = ApplyFlowName.getApplyFlowName(key);
	        	builder.name(flowName==null?flowName:key);
	        	builder.key(key);
	        	builder.enableDuplicateFiltering();
	        	builder.tenantId(LoginUserHelper.getCurrentLoginUser().getTenantId());
	        	builder.deploy();
	        //}
    	//}
    }
    public static void main(String[] args) {
    	System.out.println(ApplyFlowName.getApplyFlowName("BeforSubsidize"));
	}
    /**
    ...
    /**
     * 第二种：读取resource/processes目录下的流程描述文件
     * @param resourceLoader
     * @return
     * @throws Exception 
     */
    public List<String> loadBpmnFile(ResourceLoader resourceLoader) throws Exception{
    	...
    	if(processKeys.size() == 0){
        	logger.debug("can not find .bpmn or .png in classpath:/processes/ !");
        	throw new Exception("can not find .zip or .bar in classpath:/processes/ !");
        }
    	return processKeys;
    }
