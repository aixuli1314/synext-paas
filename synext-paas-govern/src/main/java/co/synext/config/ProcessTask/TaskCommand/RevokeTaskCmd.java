package co.synext.config.ProcessTask.TaskCommand;
...
import co.synext.module.Activiti.util.ApplicationContextHelper;
...
public class RevokeTaskCmd implements Command<Integer> {
	...
	public Integer execute(CommandContext commandContext) {
    	...
    	HistoricTaskInstanceEntity historicTaskInstanceEntity = Context
                .getCommandContext()
                .getHistoricTaskInstanceEntityManager().findById(historyTaskId);
    	// 获得历史节点
...
//        this.deleteActiveTasks(processInstance.getProcessInstanceId());
    ...
    public void processHistoryTask(
        ...
        task.setDescription(historicTaskInstanceEntity.getDescription());

        Context.getCommandContext().getTaskEntityManager().insert(task);

     // 把流程指向任务对应的节点
        ExecutionEntity executionEntity = Context.getCommandContext()
                .getExecutionEntityManager().findById(historicTaskInstanceEntity.getExecutionId());
        //未完待续。。。
        //executionEntity.setActivity(getActivity(historicActivityInstanceEntity));
    }
    
	/*
	 * public ActivityImpl getActivity( HistoricActivityInstance
	 * historicActivityInstanceEntity) { ProcessDefinitionEntity
	 * processDefinitionEntity = new GetDeploymentProcessDefinitionCmd(
