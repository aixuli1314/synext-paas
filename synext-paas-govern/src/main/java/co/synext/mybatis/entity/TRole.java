...
public class TRole implements Serializable {
    ...
    /**
     * 角色的部门是否私有,0否,1是,默认0.当角色私有时,菜单只使用此角色的部门权限,不再扩散到全局角色权限,
     * 用于设置特殊的菜单权限.公共权限时部门主管有所管理部门的数据全权限,无论角色是否分配. 私有部门权限时,
     * 严格按照配置的数据执行,部门主管可能没有部门权限.
     */
    ...
    /**
     * 角色的归属部门,只有归属部门的主管和上级主管才可以管理角色,其他人员只能增加归属到角色的人员.
     * 不能选择部门或则其他操作,只能添加人员,不然存在提权风险,例如 员工角色下有1000人, 如果给 角色 设置了部门,
     * 那这1000人都起效了.
     */
    ...
    /**
     * 角色是否共享,0否 1是,默认0,共享的角色可以被下级部门直接使用,但是下级只能添加人员,不能设置其他属性.
     * 共享的角色一般只设置roleOrgType,并不设定部门.
     */
     ...
     * 排序,查询时倒叙排列
     */
    @ApiModelProperty(value = "排序,查询时倒叙排列")
